...
node {
    stage('Checkout') {
        ...
        def response = httpRequest acceptType: 'APPLICATION_JSON', consoleLogResponseBody: true, customHeaders: [[maskValue: true, name: 'Authorization', value: 'Bearer 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']], responseHandle: 'NONE', url: 'https://api.github.com/repos/mrunalgosar/gh-auto-labeler/pulls/2', wrapAsMultipart: false
        def json = new JsonSlurper().parseText(response.content)
        echo "Labels: ${json.labels[0].name}"
    }
}