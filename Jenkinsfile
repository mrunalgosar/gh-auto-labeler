...
node {
    stage('Checkout') {
        ...
        withCredentials([string(credentialsId: 'postman', variable: 'TOKEN')]) {
            def response = httpRequest acceptType: 'APPLICATION_JSON', authentication: 'postman', consoleLogResponseBody: false, responseHandle: 'NONE', url: 'https://api.github.com/repos/mrunalgosar/gh-auto-labeler/pulls/2', wrapAsMultipart: false
